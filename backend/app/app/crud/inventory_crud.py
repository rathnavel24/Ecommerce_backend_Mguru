...
class InventoryDetails:
    ...
    def create_inventory(self):
        product_check = self.db.query(EcommerceProductInfo).filter(EcommerceProductInfo.product_id == self.inventory_data.product_id).first()

        if not product_check :

            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="no product found under this id")

        
        product_check = self.db.query(EcommerceInventory).filter(EcommerceInventory.product_id == self.inventory_data.product_id).first()
